...
export default function FarmerHeader({ variant = 'default' }: FarmerHeaderProps) {
  const farmerAvatar = "https://i.pravatar.cc/150?img=12"; // mock avatar
    ...
    return (
      <header className="flex justify-between items-center w-full px-[16px] h-16 sticky top-0 z-40 bg-[#FAFAF7] border-b border-stone-200">
        <div className="flex items-center gap-3">
          <Image src={farmerAvatar} width={40} height={40} className="rounded-full object-cover border-2 border-surface" alt="Farmer" />
          <div>
            <p className="text-[12px] font-semibold tracking-[0.03em] text-on-surface-variant">Good morning,</p>
            <h1 className="text-[20px] font-semibold leading-[1.4] text-on-background">Ramon</h1>
          </div>
        </div>
  ...
  return (
    <header className="flex justify-between items-center w-full px-4 h-16 sticky top-0 z-40 bg-[#FAFAF7] border-b border-stone-200 md:hidden">
      <div className="flex items-center gap-3">
        <Image src={farmerAvatar} width={32} height={32} className="rounded-full object-cover" alt="Farmer" />
        <h1 className="text-[#1B4332] font-black text-xl tracking-tighter">FarmFlow</h1>
      </div>
